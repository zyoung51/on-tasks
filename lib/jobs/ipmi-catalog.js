...
function ipmiCatalogJobFactory(
    ...
    IpmiCatalogJob.prototype._run = function() {
        ...
        // Get plain text of obm setting
        var obmConfigFound = Promise.resolve();
        obmConfigFound = waterline.nodes.findByIdentifier(self.nodeId)
        .then(function (node) {
        ...
        obmConfigFound.then(function(obmSetting) {
            return Promise.all(_.map(self.commands, function(cmd) {
                return self.runCommand(obmSetting, cmd);
            }));
        })
        .spread(self.handleResponse.bind(self))
        .then(function() {
            self._done();
        }, function(e) {
            self._done(e);
     ...
     * @memberOf IpmiCommandJob
     */
    IpmiCatalogJob.prototype.handleResponse = function(/*... */) {
        var self = this;
        var result = _.toArray(arguments); 

        parser.parseTasks(result)
        .spread(function() {
