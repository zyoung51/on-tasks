...
function iscDhcpLeasePollerFactory(
    ...
    IscDhcpLeasePollerJob.prototype._updateLeasesCallback = function(err, data) {
        ...
        // This is primarily a readFile error and we can notify then try again.
        if (err) {
            logger.error(err.message, { error: err });
        } else {
            Promise.try(function () {
                return self.updateLookups(
                    self.parseLeaseData(data.toString())
                );
    ...
    IscDhcpLeasePollerJob.prototype.updateLookups = function(leases) {
        ...
        return Promise.settle(_.map(newLeases, function(mac, ip) {
            return lookupService.setIpAddress(ip, mac);
        })).map(function (result) {
            if (result.isRejected()) {
                logger.error('ISC Lease Poller: Error in setIpAddress.',
